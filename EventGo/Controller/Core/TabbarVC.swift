...
private extension TabbarVC {
    ...
    func createEventListVC() -> UINavigationController {
        ...
        nav.tabBarItem = UITabBarItem(title: nil, image: UIImage(systemName: "calendar"), tag: 2)
        return nav
    }
    func createEventsMapVC() -> UINavigationController {
        ...
        nav.tabBarItem = UITabBarItem(title: nil, image: UIImage(systemName: "map"), tag: 3)
        return nav
    }
    func createCalendarVC() -> UINavigationController {
        let calendarVC = CalendarVC()
        let nav = UINavigationController(rootViewController: calendarVC)
        nav.tabBarItem = UITabBarItem(title: nil, image: UIImage(systemName: "calendar.day.timeline.left"), tag: 4)
        return nav
    }
    func createProfileVC() -> UINavigationController {
        let profileVC = ProfileVC()
        let nav = UINavigationController(rootViewController: profileVC)
        nav.tabBarItem = UITabBarItem(title: nil, image: UIImage(systemName: "person.fill"), tag: 5)
        return nav
    }
}
